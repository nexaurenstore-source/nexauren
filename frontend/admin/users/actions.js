(() => {
  ...
  function isOwnRow(row){ if(!row)return false; if(row.dataset.self==='1')return true; const id=row.dataset.userId||row.querySelector('[data-id]')?.dataset.id||''; const email=row.cells?.[1]?.textContent?.trim().toLowerCase()||''; return (!!currentAdminId && id===String(currentAdminId)) || (!!currentAdminEmail && email===currentAdminEmail); }
  function protectOwnRow(row){ if(!isOwnRow(row))return; row.dataset.self='1'; const actions=row.querySelector('.actions'); if(actions) actions.innerHTML='<span class="self-protected" title="Your administrator account is protected">🔐 Protected</span>'; row.querySelectorAll('[data-view],[data-menu],[data-extra-action]').forEach(el=>el.remove()); }
  function protectAllRows(){ root.querySelectorAll('tr').forEach(protectOwnRow); }
  ...
  function addControls(){ root.querySelectorAll('.menu-panel').forEach(panel=>{const row=panel.closest('tr');if(isOwnRow(row)){protectOwnRow(row);return;}if(panel.querySelector('[data-extra-action]'))return;const item=panel.querySelector('[data-action="view"]');if(!item)return;const id=item.dataset.id;panel.insertAdjacentHTML('beforeend','<button class="menu-item" type="button" data-extra-action="edit" data-id="'+esc(id)+'">Edit user</button><button class="menu-item" type="button" data-extra-action="block" data-id="'+esc(id)+'">Temporary block</button><button class="menu-item" type="button" data-extra-action="delete" data-id="'+esc(id)+'">Delete permanently</button>');});protectAllRows(); }
