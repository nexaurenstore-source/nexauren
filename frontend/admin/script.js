...
async function createPayPalPlan(event){
  ...
  const price=Number($('paypal-plan-price').value);const replaceExisting=$('paypal-plan-replace')?.checked===true;const payload={plan_id:$('paypal-plan-id').value.trim(),product_id:$('paypal-plan-product').value.trim(),name:$('paypal-plan-name').value.trim(),description:$('paypal-plan-description').value.trim(),price_minor:Math.round(price*100),currency:$('paypal-plan-currency').value,interval_unit:$('paypal-plan-interval').value,interval_count:Number($('paypal-plan-count').value),trial_days:Number($('paypal-plan-trial').value),credits_per_cycle:Number($('paypal-plan-credits').value),replace_existing:replaceExisting};
  result.className='result';result.textContent=replaceExisting?'Migrating PayPal plan to Live…':'Creating PayPal subscription plan…';button.disabled=true;
  if(replaceExisting&&!confirm('This will create a new PayPal Live plan and replace the existing PayPal Plan ID for this local plan. Continue?')){button.disabled=false;result.textContent='Migration cancelled.';return}
  try{const r=await fetch('/api/admin/paypal/plans',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify(payload)});const data=await r.json().catch(()=>({}));if(r.status===401||r.status===403)throw new Error(data.error||'Administrator access required.');if(!r.ok)throw new Error(data.error||'Unable to create the PayPal plan.');const plan=data.plan||{};result.className='result success';result.innerHTML=`<strong>${replaceExisting?'Live plan migrated successfully.':'Plan created successfully.'}</strong><br>Local plan: <code>${escapeHtml(plan.id||'—')}</code><br>PayPal Plan ID: <code>${escapeHtml(plan.paypal_plan_id||'—')}</code>`;$('paypal-plan-form').reset();$('paypal-plan-count').value='1';$('paypal-plan-trial').value='0';$('paypal-plan-credits').value='1000';$('paypal-plan-currency').value='USD';$('paypal-plan-interval').value='MONTH';}catch(err){result.className='result error';result.textContent=err.message||'Unable to create the PayPal plan.'}finally{button.disabled=false}
}
