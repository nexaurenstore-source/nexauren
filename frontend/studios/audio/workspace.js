(() => {
  ...
  const STORE = 'nexauren-audio-workspace';
  ...
  const saveFile = async f => { try { const db = await openStore(); db.transaction('files', 'readwrite').objectStore('files').put(f, 'current'); } catch {} };
  const loadFile = async () => { try { const db = await openStore(); return await new Promise((resolve, reject) => { const r = db.transaction('files').objectStore('files').get('current'); r.onsuccess = () => resolve(r.result || null); r.onerror = () => reject(r.error); }); } catch { return null; } };
  const clearSavedFile = async () => { try { const db = await openStore(); db.transaction('files', 'readwrite').objectStore('files').delete('current'); } catch {} };
  ...
  const updateSelection = () => {
    ...
    const ss = duration * s / 100;
    const ee = duration * e / 100;
  ...
  const previewSelection = async () => {
    if (!audioBuffer) return;
    const s = Number($('start').value) / 100;
    const e = Number($('end').value) / 100;
    ...
    status.textContent = 'Previewing selection.';
    previewAudio.onended = () => { status.textContent = 'Ready to process.'; };
  };
  ...
  const setFile = async (f, persist = true) => {
    ...
    try { audioBuffer = await decode(f); $('wave').style.display='block'; drawWave(audioBuffer); processBtn.disabled=false; status.textContent='Ready to process.'; updateSelection(); }
  ...
  $('resetBtn')?.addEventListener('click',()=>{clearSavedFile();file=null;audioBuffer=null;if(previewAudio){previewAudio.pause();previewAudio=null;}fileInput.value='';fileName.textContent='No file selected';fileMeta.textContent='';processBtn.disabled=true;result.classList.remove('show');$('wave').style.display='none';status.textContent='Choose an audio file to begin.'});
  ...
  loadFile().then(saved => { if (saved && !file) setFile(saved, false); });
})();
