(() => {
  ...
  const saveFile = async f => {
    try { const db = await openStore(); const tx = db.transaction('files', 'readwrite'); tx.objectStore('files').put(f, 'current'); } catch {}
  };
  ...
  $('resetBtn')?.addEventListener('click',()=>{clearSavedFile();file=null;audioBuffer=null;fileInput.value='';fileName.textContent='No file selected';fileMeta.textContent='';processBtn.disabled=true;result.classList.remove('show');$('wave').style.display='none';status.textContent='Choose an audio file to begin.'});
