...
async function refreshUsage(){try{const r=await fetch('/api/tools/sample-maker/usage',{method:'GET',credentials:'include',cache:'no-store',headers:{Accept:'application/json'}});if(!r.ok)return;const data=await r.json();if(data.plan&&limits[data.plan]){currentPlan=data.plan;window.NEXAUREN_SAMPLE_MAKER_APPLY_PLAN?.(data.plan)}if(data.daily_limit!=null)window.NEXAUREN_SAMPLE_MAKER_SET_USAGE?.(data.remaining)}catch(e){console.warn('Sample Maker usage lookup failed',e)}}
