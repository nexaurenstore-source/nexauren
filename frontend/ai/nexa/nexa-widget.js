(() => {
  ...
  root.innerHTML = `
    ...
    <div class="nexa-panel" hidden>
      ...
      <div class="nexa-suggestions"><button>Find an image tool</button><button>Create an image of a futuristic African city</button><button>Help me write</button></div>
  ...
  function addImageAction(action){
    ...
    box.className='nexa-agent-result';
    const img=document.createElement('img');
    img.src=result.image;
    img.alt=result.prompt||'Nexa AI generated image';
    img.loading='lazy';
    const caption=document.createElement('div');
    caption.className='nexa-agent-caption';
    caption.textContent='Image generated by Nexa AI';
    const link=document.createElement('a');
    link.href=result.image;
    link.download=result.filename||'nexauren-image.jpg';
    link.textContent='Download image';
    link.className='nexa-agent-download';
    box.appendChild(img); box.appendChild(caption); box.appendChild(link);
  ...
  async function send(){if(busy)return;const text=input.value.trim();if(!text)return;busy=true;input.value='';add('user',text);const loading=add('assistant','',true);try{const r=await fetch('/api/nexa-ai/chat',{method:'POST',credentials:'include',headers:{'content-type':'application/json'},body:JSON.stringify({message:text,conversation_id:conversationId,web,analysis,tools:true,agent:true})});const d=await r.json();loading.remove();if(!r.ok){add('assistant',d.error||'Something went wrong.');if(d.code==='feature_plan_required')loadLimits();return;}conversationId=d.conversation_id;add('assistant',d.answer);if(d.agent_action){addImageAction(d.agent_action);}if(d.web_sources?.length){const box=document.createElement('div');box.className='nexa-sources';box.innerHTML='<strong>Sources</strong>'+d.web_sources.map(x=>`<a href="${esc(x.url)}" target="_blank" rel="noopener">${esc(x.title)}</a>`).join('');messages.appendChild(box);}if(d.tools?.length){const box=document.createElement('div');box.className='nexa-tools-found';box.innerHTML='<strong>Nexauren tools</strong>'+d.tools.map(x=>`<a href="/tools/${esc(x.slug)}/">${esc(x.name)}</a>`).join('');messages.appendChild(box);}loadLimits();}catch(e){loading.remove();add('assistant','I could not reach Nexa AI right now. Please try again.');}finally{busy=false;input.focus();}}
