...
async function adminUserBlock(r, e) {
  ...
  if (!user) return json({ error: 'User not found.' }, 404, cors(r));
  const duration = Math.floor(Number((await body(r))?.duration_seconds));
...
console.log('[worker-check] Admin Users extensions applied without modifying login verification.');
