...
for (const route of ['/api/billing/account','/api/billing/transactions','/api/billing/catalog','/api/billing/checkout']) if (!billing.includes(route)) throw new Error(`[billing-ui] Missing API integration: ${route}`);
if (!billing.includes('/api/billing/subscription/'+"action") && !billing.includes('/api/billing/subscription/')) throw new Error('[billing-ui] Missing subscription action API integration.');
