...
async function adminBlockedUsers(r, e) {
  ...
  const q = clean(u.searchParams.get('q') || '').slice(0, 120);
  const like = '%' + q.replace(/[%_]/g, '\\$&') + '%';
  const where = q ? 'WHERE (u.username LIKE ?1 ESCAPE \\\\'\\\\\\\' OR u.email LIKE ?1 ESCAPE \\\\'\\\\\\\')' : '';
  const countStmt = q
    ? e.DB.prepare('SELECT COUNT(*) AS total FROM admin_user_blocks b JOIN users u ON u.id=b.user_id WHERE (u.username LIKE ?1 ESCAPE \\\\'\\\\\\\' OR u.email LIKE ?1 ESCAPE \\\\'\\\\\\\')').bind(like)
    : e.DB.prepare('SELECT COUNT(*) AS total FROM admin_user_blocks');
  const listStmt = q
    ? e.DB.prepare('SELECT u.id,u.email,u.username,u.created_at,u.updated_at,b.blocked_at,COALESCE(p.xp,0) AS xp,COALESCE(p.level,1) AS level FROM admin_user_blocks b JOIN users u ON u.id=b.user_id LEFT JOIN user_progress p ON p.user_id=u.id WHERE (u.username LIKE ?1 ESCAPE \\\\'\\\\\\\' OR u.email LIKE ?1 ESCAPE \\\\'\\\\\\\') ORDER BY b.blocked_at DESC LIMIT ?2 OFFSET ?3').bind(like, limit, offset)
