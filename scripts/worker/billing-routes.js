...
if (__billingUrl.pathname === '/api/billing/subscription/cancel' && r.method === 'POST') {
  ...
  if (clean(sub.provider).toLowerCase() === 'paypal') {
    ...
    catch (error) { console.error('PayPal subscription cancellation failed', String(error).slice(0, 500)); return json({ error: 'Unable to cancel the subscription with the payment provider.' }, 502, cors(r)); }
  }
...
if (__billingUrl.pathname === '/api/billing/subscription/resume' && r.method === 'POST') {
  ...
  if (Number(sub.cancel_at_period_end) === 1 && clean(sub.provider).toLowerCase() === 'paypal') {
    ...
    catch (error) { console.error('PayPal subscription resume failed', String(error).slice(0, 500)); return json({ error: 'Unable to resume the subscription with the payment provider.' }, 502, cors(r)); }
  }
