...
if (!moduleSource.includes('async function sampleMakerGeneration(')) {
...
if (count(generated, 'async function sampleMakerGeneration(') !== 1) {
  throw new Error('[sample-maker-build] Sample Maker enforcement must be included exactly once.');
}
if (count(generated, 'const __sampleMakerUrl') !== 1) {
  throw new Error('[sample-maker-build] Sample Maker route must be included exactly once.');
}
...
console.log('[sample-maker-build] Sample Maker route included once.');
