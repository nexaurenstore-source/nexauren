...
const module = String.raw`
    ...
    const __communityLegacyToolFavorite = async (r, e) => {
      const path = new URL(r.url).pathname.split('/').filter(Boolean);
      const toolId = clean(path[2]);
      if (path.length !== 3 || path[1] !== 'favorites' || !toolId) return null;
      ...
      const tool = tools.find((item) => String(item?.id || '') === toolId || String(item?.slug || '') === toolId);
      ...
      if (r.method === 'GET') {
        ...
        const row = await e.DB.prepare('SELECT 1 FROM tool_favorites WHERE user_id=?1 AND tool_id=?2 LIMIT 1').bind(u.id, String(tool.id)).first();
      ...
      if (r.method === 'DELETE') {
        await e.DB.prepare('DELETE FROM tool_favorites WHERE user_id=?1 AND tool_id=?2').bind(u.id, String(tool.id)).run();
    ...
    const __communityToolTarget = async (r, e) => {
      ...
      if (section === 'ratings') {
        ...
        if (r.method === 'POST') {
          const u = await currentUser(r, e); if (!u) return json({ error: 'Authentication required.' }, 401, cors(r));
          const d = await body(r), rating = Math.floor(Number(d?.rating)), review = clean(d?.review).slice(0, 1000), displayName = clean(d?.display_name || u.username).slice(0, 50);
      ...
      if (r.method === 'GET') {
        const u = await currentUser(r, e); if (!u) return json({ favorite: false, authenticated: false }, 200, cors(r));
      ...
      const u = await currentUser(r, e); if (!u) return json({ error: 'Authentication required.' }, 401, cors(r));
      if (r.method === 'POST') { const now = Math.floor(Date.now() / 1000); await e.DB.prepare('INSERT OR IGNORE INTO favorites (user_id,target_type,studio_id,experience_id,url,created_at) VALUES (?1,?2,?3,?4,?5,?6)').bind(u.id, dbTargetType, dbStudioId, dbExperienceId, toolUrl, now).run(); return json({ favorite: true, target_type: 'tool', tool_id: toolId }, 201, cors(r)); }
      if (r.method === 'DELETE') { await e.DB.prepare('DELETE FROM favorites WHERE user_id=?1 AND target_type=?2 AND studio_id=?3 AND experience_id=?4').bind(u.id, dbTargetType, dbStudioId, dbExperienceId).run(); return json({ favorite: false, target_type: 'tool', tool_id: toolId }, 200, cors(r)); }
      return json({ error: 'Method not allowed.' }, 405, cors(r));
    };
    const __communityToolPath = new URL(r.url).pathname;
    if (__communityToolPath.startsWith('/api/favorites/') && __communityToolPath.split('/').filter(Boolean).length === 3) {
      const __legacyFavoriteResponse = await __communityLegacyToolFavorite(r, e);
      if (__legacyFavoriteResponse) return __legacyFavoriteResponse;
    }
    if (__communityToolPath.startsWith('/api/ratings/tool/') || __communityToolPath.startsWith('/api/favorites/tool/')) {
...
console.log('[community-tools] Tool ratings, legacy tool favorites and unified tool favorites routes patched.');
