...
async function paypalCreateCheckout({ env, user, reference, product, productType }) {
  ...
  if (productType === 'subscription') {
    ...
    const response = await fetch(`${base}/v1/billing/subscriptions`, {
      method: 'POST', headers: paypalHeaders(token),
      body: JSON.stringify({
        plan_id: planId,
        custom_id: reference,
        subscriber: { email_address: user.email },
        application_context: { brand_name: clean(env.PAYMENT_BRAND_NAME) || 'Nexauren', user_action: 'SUBSCRIBE_NOW', return_url: returnUrl, cancel_url: cancelUrl },
      }),
    });
    const data = await providerJson(response);
  ...
  const response = await fetch(`${base}/v2/checkout/orders`, {
    ...
    body: JSON.stringify({
      intent: 'CAPTURE',
      purchase_units: [{ reference_id: reference, custom_id: reference, amount: { currency_code: String(product.currency).toUpperCase(), value: (Number(product.price_minor) / 100).toFixed(2) } }],
      application_context: { brand_name: clean(env.PAYMENT_BRAND_NAME) || 'Nexauren', user_action: 'PAY_NOW', return_url: returnUrl, cancel_url: cancelUrl },
    }),
  });
  const data = await providerJson(response);
  const approve = (data?.links || []).find((link) => link.rel === 'approve');
...
async function paypalVerifyWebhook(env, request, event) {
  ...
  const payload = {
    auth_algo: h.get('paypal-auth-algo'), cert_url: h.get('paypal-cert-url'), transmission_id: h.get('paypal-transmission-id'),
    transmission_sig: h.get('paypal-transmission-sig'), transmission_time: h.get('paypal-transmission-time'), webhook_id: webhookId, webhook_event: event,
  };
  if (Object.values(payload).some((value) => !value)) throw new Error('Incomplete PayPal webhook signature headers.');
  const token = await paypalAccessToken(env);
  const response = await fetch(`${paypalBase(env)}/v1/notifications/verify-webhook-signature`, { method: 'POST', headers: paypalHeaders(token), body: JSON.stringify(payload) });
  const data = await providerJson(response);
...
async function paypalHandleWebhook({ request, env, finalize }) {
  ...
  if (event?.event_type !== 'CHECKOUT.ORDER.APPROVED') return json({ received: true, ignored: true }, 200);

  const orderId = clean(event?.resource?.id);
  if (!orderId) return json({ received: true }, 200);
  const token = await paypalAccessToken(env);
  const capture = await fetch(`${paypalBase(env)}/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`, { method: 'POST', headers: paypalHeaders(token) });
  ...
  if (String(payment?.amount?.currency_code).toUpperCase() !== String(row.currency).toUpperCase() || paid < expected) throw new Error('PayPal payment verification mismatch.');
  await finalize({ provider: 'paypal', reference, providerTransactionId: payment.id, status: 'successful', userId: row.user_id, amountMinor: row.amount_minor, currency: row.currency, type: row.type, productId: meta.product_id, metadata: { ...meta, paypal_order_id: orderId, paypal_capture_id: payment.id } });
...
function createPayPalProvider() { return Object.freeze({ name: 'paypal', createCheckout: paypalCreateCheckout, handleWebhook: paypalHandleWebhook }); }
