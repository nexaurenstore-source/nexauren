...
async function billingRecordWebhook(e, { provider, eventId, eventType, payload }) {
  const id = `${clean(provider)}:${clean(eventId)}`;
  const now = Math.floor(Date.now() / 1000);
  const result = await e.DB.prepare(
    'INSERT OR IGNORE INTO webhook_events(id,provider,event_id,event_type,payload,status,created_at,processed_at) VALUES(?1,?2,?3,?4,?5,\'received\',?6,NULL)',
  ).bind(id, provider, eventId, eventType, JSON.stringify(payload).slice(0, 50000), now).run();
...
async function billingWebhook(r, e, providerName) {
  ...
  const eventId = r.headers.get('paypal-transmission-id') || r.headers.get('flutterwave-signature') || `request:${crypto.randomUUID()}`;
  const eventType = 'provider.webhook';
  const raw = await r.clone().text();
  let payload = null;
  try { payload = JSON.parse(raw); } catch { throw new Error('Invalid webhook JSON.'); }
  const recorded = await billingRecordWebhook(e, { provider, eventId, eventType, payload });
