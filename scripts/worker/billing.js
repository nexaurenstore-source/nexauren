...
function billingProviderRegistry(e) {
  if (typeof buildPaymentProviderRegistry === 'function') return buildPaymentProviderRegistry(e);
  return globalThis.__NEXAUREN_PAYMENT_PROVIDERS || Object.freeze({});
}

async function billingEnsureAccount(e, userId) {
  const now = Math.floor(Date.now() / 1000);
  const freePlan = await e.DB.prepare("SELECT id,credits_per_cycle FROM plans WHERE id='free' AND enabled=1 LIMIT 1").first();
  if (!freePlan) throw new Error('Billing catalog is not initialized.');
  const reference = `grant:free:${userId}`;
  const freeCredits = Math.max(0, Number(freePlan.credits_per_cycle || 0));
  await e.DB.batch([
    e.DB.prepare('INSERT OR IGNORE INTO billing_accounts(user_id,plan_id,created_at,updated_at) VALUES(?1,?2,?3,?3)').bind(userId, 'free', now),
...
async function billingCatalog(r, e) {
  const [plans, packages] = await Promise.all([
    e.DB.prepare('SELECT id,name,price_minor,currency,billing_interval,credits_per_cycle FROM plans WHERE enabled=1 ORDER BY price_minor ASC').all(),
    e.DB.prepare('SELECT id,name,credits,price_minor,currency FROM credit_packages WHERE enabled=1 ORDER BY credits ASC').all(),
  ]);
  const providerName = clean(e.PAYMENT_PROVIDER).toLowerCase();
  const registry = billingProviderRegistry(e);
  return json({ provider: providerName || null, checkout_ready: !!registry[providerName], plans: plans?.results || [], credit_packages: packages?.results || [] }, 200, cors(r));
}
...
async function billingTransactions(r, e) {
  ...
  const [count, rows] = await Promise.all([
    e.DB.prepare('SELECT COUNT(*) AS total FROM credit_transactions WHERE user_id=?1').bind(u.id).first(),
    e.DB.prepare('SELECT id,amount,type,description,reference,payment_id,tool_id,created_at FROM credit_transactions WHERE user_id=?1 ORDER BY created_at DESC LIMIT ?2 OFFSET ?3').bind(u.id, limit, offset).all(),
  ]);
  return json({ page, limit, total: Number(count?.total || 0), transactions: rows?.results || [] }, 200, cors(r));
}
...
async function billingCheckout(r, e) {
  ...
  if (!BILLING_PRODUCT_TYPES.has(productType) || !productId) return json({ error: 'Invalid billing product.' }, 400, cors(r));
  const providerName = clean(e.PAYMENT_PROVIDER).toLowerCase();
  ...
  const table = productType === 'credit_purchase' ? 'credit_packages' : 'plans';
  const product = await e.DB.prepare(`SELECT * FROM ${table} WHERE id=?1 AND enabled=1 LIMIT 1`).bind(productId).first();
  ...
  try {
    ...
    await e.DB.prepare('UPDATE payments SET provider_transaction_id=?1,metadata=?2,updated_at=?3 WHERE id=?4 AND status=\'pending\'').bind(providerTransactionId || null, JSON.stringify({ product_id: productId, checkout_mode: checkout?.mode || null }), now, paymentId).run();
    return json({ success: true, provider: providerName, reference, checkout }, 201, cors(r));
  } catch (err) {
    await e.DB.prepare("UPDATE payments SET status='failed',metadata=?1,updated_at=?2 WHERE id=?3 AND status='pending'").bind(JSON.stringify({ product_id: productId, error: String(err).slice(0, 500) }), Math.floor(Date.now() / 1000), paymentId).run();
    console.error('Billing checkout failed', String(err).slice(0, 500));
    return json({ error: 'Unable to create checkout.' }, 502, cors(r));
  }
}

async function billingAddCredits(e, { userId, amount, type, description, reference, paymentId = null, toolId = null }) {
  ...
  const result = await e.DB.batch([
    e.DB.prepare("INSERT OR IGNORE INTO credit_transactions (id,user_id,amount,type,description,reference,payment_id,tool_id,created_at) VALUES(?1,?2,?3,?4,?5,?6,?7,?8,?9)").bind(uuid(), userId, credits, type, clean(description).slice(0, 240), reference, paymentId, toolId, now),
    e.DB.prepare('INSERT OR IGNORE INTO credit_balances(user_id,balance,updated_at) VALUES(?1,0,?2)').bind(userId, now),
    e.DB.prepare('UPDATE credit_balances SET balance=(SELECT COALESCE(SUM(amount),0) FROM credit_transactions WHERE user_id=?1),updated_at=?2 WHERE user_id=?1').bind(userId, now),
  ]);
...
async function billingDebitCredits(e, { userId, amount, toolId, reference }) {
  const credits = Math.floor(Number(amount));
  if (!Number.isFinite(credits) || credits <= 0) throw new Error('Invalid credit amount.');
  const now = Math.floor(Date.now() / 1000);
  const result = await e.DB.batch([
    e.DB.prepare("INSERT OR IGNORE INTO credit_transactions (id,user_id,amount,type,description,reference,payment_id,tool_id,created_at) SELECT ?1,?2,?3,'usage',?4,?5,NULL,?6,?7 WHERE NOT EXISTS(SELECT 1 FROM credit_transactions WHERE reference=?5) AND EXISTS(SELECT 1 FROM credit_balances WHERE user_id=?2 AND balance>=?8)").bind(uuid(), userId, -credits, `Tool usage: ${clean(toolId).slice(0, 120)}`, reference, toolId, now, credits),
    e.DB.prepare('INSERT OR IGNORE INTO credit_balances(user_id,balance,updated_at) VALUES(?1,0,?2)').bind(userId, now),
    e.DB.prepare('UPDATE credit_balances SET balance=(SELECT COALESCE(SUM(amount),0) FROM credit_transactions WHERE user_id=?1),updated_at=?2 WHERE user_id=?1').bind(userId, now),
  ]);
...
async function billingUsage(r, e) {
  ...
  const tool = await e.DB.prepare('SELECT tool_id,credit_cost,enabled FROM tool_billing WHERE tool_id=?1 LIMIT 1').bind(toolId).first();
  ...
  if (!Number.isFinite(cost) || cost < 0) return json({ error: 'Invalid tool credit cost.' }, 500, cors(r));
  if (cost === 0) return json({ success: true, charged: 0, reference }, 200, cors(r));
  const result = await billingDebitCredits(e, { userId: u.id, amount: cost, toolId, reference });
  if (result.insufficient) {
    const account = await billingEnsureAccount(e, u.id);
    return json({ error: 'Insufficient credits.', code: 'insufficient_credits', balance: Number(account?.balance || 0), required: cost }, 402, cors(r));
  }
  const account = await billingEnsureAccount(e, u.id);
  return json({ success: true, charged: result.idempotent ? 0 : cost, idempotent: !!result.idempotent, reference, balance: Number(account?.balance || 0) }, 200, cors(r));
}

async function billingWebhook(r, e, providerName) {
  const name = clean(providerName).toLowerCase();
  const provider = billingProviderRegistry(e)[name];
  if (!provider || typeof provider.handleWebhook !== 'function') return json({ error: 'Payment provider not configured.', code: 'provider_required' }, 503, cors(r));
...
async function billingFinalizePayment(e, verified) {
  ...
  if (type === 'credit_purchase') {
    ...
    const product = await e.DB.prepare('SELECT credits FROM credit_packages WHERE id=?1 AND enabled=1 LIMIT 1').bind(productId).first();
    if (!product) throw new Error('Credit product not found.');
