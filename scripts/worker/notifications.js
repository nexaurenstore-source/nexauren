...
// This file is injected into the Worker build and intentionally uses the
// Worker runtime helpers (uuid, clean, body, json, cors, isAdmin, currentUser).
...
async function createAdminNotification(r, e) {
  ...
  if (d?.publish_immediately === true) { status = 'SENT'; publishedAt = now; }
  else if (d?.scheduled_at) { const scheduled = new Date(d.scheduled_at).getTime(); if (!Number.isFinite(scheduled)) return json({ error: 'Invalid scheduled_at.' }, 400, cors(r)); status = 'ACTIVE'; publishedAt = Math.max(now, Math.floor(scheduled / 1000)); }
...
async function publishAdminNotification(r, e, id) {
  ...
  const n = await e.DB.prepare('SELECT * FROM notifications WHERE id=?1 LIMIT 1').bind(id).first();
  if (!n) return json({ error: 'Notification not found.' }, 404, cors(r));
  if (n.status === 'ARCHIVED' || n.status === 'SENT') return json({ error: 'Notification is already archived or sent.' }, 409, cors(r));
  ...
  const now = Math.floor(Date.now() / 1000);
  await e.DB.prepare("UPDATE notifications SET status='SENT',published_at=?1,updated_at=?1 WHERE id=?2").bind(now, id).run();
  await notificationAudit(e, id, admin.id, 'PUBLISHED', { recipients: count, audience });
  return json({ success: true, recipients: count, status: 'SENT' }, 200, cors(r));
}
