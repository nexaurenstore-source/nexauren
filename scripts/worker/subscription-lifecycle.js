...
async function billingProcessSubscriptionCycle(e, { provider, subscriptionId, providerTransactionId, periodStart, periodEnd, amountMinor, currency, reference }) {
  const sub = await e.DB.prepare('SELECT s.id,s.user_id,s.plan_id,s.status,s.cancel_at_period_end,p.credits_per_cycle,p.price_minor,p.currency,p.billing_interval FROM subscriptions s JOIN plans p ON p.id=s.plan_id WHERE s.id=?1 LIMIT 1').bind(subscriptionId).first();
  ...
  const now = Math.floor(Date.now() / 1000);
  const existingCycle = await e.DB.prepare('SELECT id,status,credit_transaction_id FROM subscription_cycles WHERE subscription_id=?1 AND cycle_key=?2 LIMIT 1').bind(sub.id, cycleKey).first();
  ...
  const statements = [
    e.DB.prepare("INSERT INTO subscription_cycles(id,subscription_id,user_id,cycle_key,period_start,period_end,credits,status,credit_transaction_id,created_at,processed_at) VALUES(?1,?2,?3,?4,?5,?6,?7,'credited',?8,?9,?9)").bind(cycleId, sub.id, sub.user_id, cycleKey, start, end, Number(sub.credits_per_cycle), null, now),
  ];
  if (!existingPayment) {
    statements.push(e.DB.prepare("INSERT INTO payments(id,user_id,provider,provider_transaction_id,reference,amount_minor,currency,status,type,metadata,created_at,updated_at) VALUES(?1,?2,?3,?4,?5,?6,?7,'successful','subscription',?8,?9,?9)").bind(paymentId, sub.user_id, clean(provider), String(providerTransactionId), reference, paidAmount, String(currency).toUpperCase(), JSON.stringify({ subscription_id: sub.id, cycle_key: cycleKey }), now));
  ...
  try {
    ...
    const committed = await e.DB.prepare('SELECT id,status,credit_transaction_id FROM subscription_cycles WHERE subscription_id=?1 AND cycle_key=?2 LIMIT 1').bind(sub.id, cycleKey).first();
    if (committed) return { processed: false, idempotent: true, cycle: committed };
    throw error;
  }
