...
for (const column of ['current_period_start','current_period_end','cancel_at_period_end','subscription_cycles','idx_subscription_cycles_key']) if (!lifecycleMigration.includes(column) && !lifecycle.includes(column)) throw new Error(`[billing] Missing lifecycle schema item: ${column}`);
