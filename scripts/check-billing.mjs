...
for (const source of [webhook,safety,routes,build,providers]) if (source.includes('FLW_SECRET_KEY=') || source.includes('PAYPAL_CLIENT_SECRET=')) throw new Error('[billing] Provider secrets must never be committed.');
for (const route of ['/api/billing/catalog','/api/billing/account','/api/billing/payment','/api/billing/transactions','/api/billing/checkout','/api/billing/usage','/api/billing/subscription/cancel','/api/billing/subscription/resume','/api/webhooks/']) if (!routes.includes(route)) throw new Error(`[billing] Missing route: ${route}`);
for (const contract of ['createCheckout','handleWebhook','buildPaymentProviderRegistry','PAYMENT_PROVIDER','subscriptionAction','paypalVerifyWebhook','paypalGetSubscription','paypalCancelSubscription','paypalResumeSubscription','PayPal-Request-Id']) if (!providers.includes(contract) && !build.includes(contract)) throw new Error(`[billing] Missing provider contract: ${contract}`);
for (const event of ['CHECKOUT.ORDER.APPROVED','PAYMENT.CAPTURE.COMPLETED','CHECKOUT.PAYMENT-APPROVAL.REVERSED','PAYMENT.SALE.COMPLETED','PAYMENT.SALE.REFUNDED','PAYMENT.SALE.REVERSED','BILLING.SUBSCRIPTION.ACTIVATED','BILLING.SUBSCRIPTION.CANCELLED','BILLING.SUBSCRIPTION.EXPIRED','BILLING.SUBSCRIPTION.SUSPENDED','BILLING.SUBSCRIPTION.PAYMENT.FAILED']) if (!providers.includes(event)) throw new Error(`[billing] Missing PayPal event handling: ${event}`);
...
if (!billing.includes('product.price_minor') || !billing.includes('product.credits') || !billing.includes('productId')) throw new Error('[billing] Backend must determine product price and credits.');
