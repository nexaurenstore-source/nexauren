...
for (const source of [webhook,safety,routes,build,providers]) if (source.includes('FLW_SECRET_KEY=') || source.includes('FLW_SECRET_HASH=') || source.includes('PAYPAL_CLIENT_SECRET=')) throw new Error('[billing] Provider secrets must never be committed.');
...
for (const contract of ['createCheckout','handleWebhook','buildPaymentProviderRegistry','PAYMENT_PROVIDER','subscriptionAction','flutterwaveCreateCheckout','flutterwaveHandleWebhook','flutterwaveVerifyTransaction','flutterwaveVerifyWebhookSignature','flutterwaveCancelSubscription','flutterwaveActivateSubscription']) if (!providers.includes(contract) && !build.includes(contract)) throw new Error(`[billing] Missing Flutterwave provider contract: ${contract}`);
for (const event of ['charge.completed','refund.completed','subscription.cancelled']) if (!providers.includes(event)) throw new Error(`[billing] Missing Flutterwave event handling: ${event}`);
...
console.log('[billing] Architecture, Flutterwave provider, webhook lifecycle, idempotency and refund checks passed.');
