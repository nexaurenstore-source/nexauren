...
const [migration, billing, safety, routes, build] = await Promise.all([
...
for (const table of requiredTables) {
  if (!migration.includes(`CREATE TABLE IF NOT EXISTS ${table}`)) {
    throw new Error(`[billing] Missing table: ${table}`);
  }
}
...
for (const invariant of [
  ...
  if (!migration.includes(invariant) && !billing.includes(invariant)) {
    throw new Error(`[billing] Missing invariant: ${invariant}`);
  }
}

for (const source of [billing, safety, routes, build]) {
  if (source.includes('FLW_SECRET_KEY') || source.includes('PAYPAL_CLIENT_SECRET')) {
    throw new Error('[billing] Provider secrets must never be committed.');
  }
}
